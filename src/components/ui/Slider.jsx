...
const CardImage = ({ img }) => {
  ...
  return (
    <>
      {img.poster_path || img.file_path || img.still_path || img.backdrop_path || img.profile_path ? <motion.img variants={cardVariant} loading='lazy' src={`${IMG_URL}/w500/${img.poster_path ?? img.file_path ?? img.still_path ?? img.backdrop_path ?? img.profile_path}`} alt={img.title ?? img.name} /> : <img src={placeholderImage} alt="there is nothing to display" />}
    </>
  )
}
...
const SliderComponent = ({ gallery, mediaType, id, size }) => {
  const renderSlideItem = (item) => {
    if (mediaType === "season") {
      ...
      return (
        <SwiperSlide key={item.id} className='swiper-slide' >
          <Link to={`/tv/${id}/season/${item.season_number}/episode/${item.episode_number}`}>
            <CardImage img={item} />
            <Overlay>
              <SlideTitle>{item.title ?? item.name}</SlideTitle>
            </Overlay>
